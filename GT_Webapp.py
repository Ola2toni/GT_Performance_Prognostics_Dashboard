...
download_file(
    "1TxQ8NCwXAB1C6csQItalGbnsbCKnTBnV",
    "saved_models/EXHAUST TEMPERATURE_ANN.pth"
)

download_file(
    "1ZOlUmZm3MvJYg2ROOckqlCfsMjEmliz2",
    "saved_models/EXHAUST TEMPERATURE_Decision Tree.pkl"
)

download_file(
    "1cPzLPbKcvJQQCaYjfgms_IH30G9vfU4N",
    "saved_models/EXHAUST TEMPERATURE_metrics.csv"
)

download_file(
    "18ytAiQKj_u70mLhibKftOubKAYhsU22G",
    "saved_models/EXHAUST TEMPERATURE_Random Forest.pkl"
)

download_file(
    "1piwjas5YojITE4x-vyRPoOiJf0OPt89z",
    "saved_models/EXHAUST TEMPERATURE_SVM.pkl"
)

download_file(
    "1y_Pink1UNKG0n73cys4TO9fNG9XVNj4u",
    "saved_models/EXHAUST TEMPERATURE_XGBoost.pkl"
)

download_file(
    "18tqMblXJxVG7FFqpia4PiaEw9GoqFRJ8",
    "saved_models/features.pkl"
)

download_file(
    "1Auj7ZsnCJ-Wt7-tVZ9Zlc2XaovIo5K_j",
    "saved_models/OUTPUT POWER_ANN.pth"
)

download_file(
    "1wHwRYfA5j3PMtkjVVD0zvq2Es84VaZ_x",
    "saved_models/OUTPUT POWER_Decision Tree.pkl"
)

download_file(
    "1E_Z9yaquwaR3N7rNiiqTjMOfsn0BWvu3",
    "saved_models/OUTPUT POWER_metrics.csv"
)

download_file(
    "1es4NX1B7xZ8eNtihNcFN21CXCBsO2-PE",
    "saved_models/OUTPUT POWER_Random Forest.pkl"
)

download_file(
    "1Dw557M8qtaKUjn0ENSRDwIOc5SS4_lzZ",
    "saved_models/OUTPUT POWER_SVM.pkl"
)

download_file(
    "1QOXBlZqTPvGkPZlPCg1D98ylUBgfEO43",
    "saved_models/OUTPUT POWER_XGBoost.pkl"
)

download_file(
    "1-K_WhgZ0gJJ83dUnC27WB9HxUMb8BGnn",
    "saved_models/scaler.pkl"
)

download_file(
    "1QnjRaGEsgkIYR6GElWHJD-_NIt0YmSq9",
    "saved_models/SFC_ANN.pth"
)

download_file(
    "1tg4mCS_f-SRDXV4ThqXGr_k8mnFw7GHS",
    "saved_models/SFC_Decision Tree.pkl"
)

download_file(
    "18owFK5jSsUmypX0q3NVxfhpF5CRtT2He",
    "saved_models/SFC_metrics.csv"
)

download_file(
    "1k3pxmmQmvPNElEY6X5AGpS9hzrx9-JZn",
    "saved_models/SFC_Random Forest.pkl"
)

download_file(
    "1dx9ouzuGRgVndga61_lYfD2ygkuHikqm",
    "saved_models/SFC_SVM.pkl"
)

download_file(
    "1n6LcsVIcFVSJu7mhQ7t8tJwb7c8XNJ3C",
    "saved_models/SFC_XGBoost.pkl"
)

download_file(
    "1zy2dwtRw1viLhBiHPpfWR3n6QKLmQpij",
    "saved_models/X_test_eff.pkl"
)

download_file(
    "1jOPhUZTEasCmSAO-Ay2eK3P1QRe-ylxC",
    "saved_models/X_test_power.pkl"
)

download_file(
    "1HApi8ikYgrp9I5dVZ9FVZkWQwpOWLwhe",
    "saved_models/X_test_sfc.pkl"
)

download_file(
    "1Zso6UCUSs-vX8yUBtJJe1OYeFv1RQZJ2",
    "saved_models/y_test_eff.pkl"
)

download_file(
    "1eGGTQAe4VXme-WVZ6DHRoQ5xVgvy5XPQ",
    "saved_models/y_test_power.pkl"
)

download_file(
    "1pYC-UmSFk-lDq-m5bqbTMT-lv-QbFWul",
...
if section == "Home":
    ...
    st.title("Data Preprocessing & Outlier Handling")
    ...
    scaler = pickle.load(
        open("saved_models/scaler.pkl", "rb")
    )

    features = pickle.load(
        open("saved_models/features.pkl", "rb")
    )
    ...
    if selected_model == "ANN":
        ...
        model.load_state_dict(
            ...
            torch.load(
                ...
                f"saved_models/{selected_target}_ANN.pth",
    ...
    metrics_path = f"saved_models/{selected_target}_metrics.csv"
    ...
    if st.button("Predict Turbine Performance"):
        ...
        for target in targets:
            ...
            metrics_path = f"saved_models/{target}_metrics.csv"
            ...
            if best_model_name == "ANN":
                ...
                model_path = f"saved_models/{target}_{best_model_name}.pkl"
        ...
        with c1:
            ...
            st.metric(
                "OUTPUT POWER",
                f"{predictions['OUTPUT POWER']['prediction']:.2f}"
            )
        ...
        with c2:
            ...
            st.metric(
                "FUEL GAS FLOW",
                f"{predictions['SFC']['prediction']:.2f}"
            )
        ...
        with c3:
            ...
            st.metric(
                "EXHAUST TEMPERATURE",
                f"{predictions['EXHAUST TEMPERATURE']['prediction']:.2f}"
            )

            # st.write(
            #     f"Best Model: {predictions['EXHAUST TEMPERATURE']['model']}"
            # )
